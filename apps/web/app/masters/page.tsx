...
export default async function MastersPage({ searchParams }: MastersPageProps) {
  ...
  const serviceTexts = {
    ...
    namePlaceholder: locale === "ru" ? "Например: Маникюр" : "For example: Consultation",
    duration: locale === "ru" ? "Длительность, мин" : "Duration, min",
    price: locale === "ru" ? "Цена" : "Price",
    pricePlaceholder: locale === "ru" ? "Пусто = укажем позже" : "Empty = set later",
  ...
  const serviceMessages: Record<string, string> = {
    ...
    service_name_required: locale === "ru" ? "Укажите название услуги" : "Please enter a service name",
    service_duration_invalid: locale === "ru" ? "Длительность услуги указана некорректно" : "Service duration is invalid",
    service_price_invalid: locale === "ru" ? "Цена услуги должна быть числом" : "Service price must be a number",
    ...
    service_remove_last_active: locale === "ru" ? "Нужно оставить хотя бы одну активную услугу" : "At least one active service must remain",
    service_has_leads: locale === "ru" ? "Эту услугу нельзя удалить: по ней уже есть заявки" : "This service cannot be deleted because it already has leads",
    service_unknown: locale === "ru" ? "Не удалось обновить услуги. Попробуйте еще раз." : "Could not update services. Please try again.",
  };
  ...
  const errorText = resolvedSearchParams.error_code
    ? dict.masters.messages[
      ...
      ] ?? serviceMessages[resolvedSearchParams.error_code] ?? dict.masters.messages.master_unknown
    : null;
  const successText = resolvedSearchParams.success_code
    ? dict.masters.messages[
      ...
      ] ?? serviceMessages[resolvedSearchParams.success_code] ?? dict.masters.messages.master_updated
    : null;
    ...
    <main className="space-y-6">
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        ...
        <section className="rounded-[2rem] border border-[color:var(--border)] bg-white p-8 shadow-[var(--shadow-md)]">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.24em] text-[color:var(--muted)]">{serviceTexts.eyebrow}</p>
              <h2 className="mt-2 text-3xl font-semibold tracking-[-0.04em] text-[color:var(--foreground)]" style={{ fontFamily: "var(--font-heading)" }}>
                {serviceTexts.addTitle}
              </h2>
            </div>
            <p className="max-w-xl text-sm leading-6 text-[color:var(--foreground-soft)]">{serviceTexts.description}</p>
          </div>

          <form action={createServiceAction} className="mt-6 grid gap-4 xl:grid-cols-[1.2fr_180px_180px_160px]">
            <input
              type="text"
              name="name"
              required
      ...
      <section className="space-y-4">
        {services.map((service) => (
          <article key={service.id} className="overflow-hidden rounded-[2rem] border border-[color:var(--border)] bg-white shadow-[var(--shadow-md)]">
            <div className={`h-1.5 ${service.isActive ? "bg-[linear-gradient(90deg,var(--accent),rgba(77,99,255,0.2))]" : "bg-[linear-gradient(90deg,rgba(132,140,166,0.8),rgba(132,140,166,0.12))]"}`} />
            <div className="p-6">
              <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                <div>
                  <p className="text-sm uppercase tracking-[0.2em] text-[color:var(--muted)]">
                    {service.isActive ? serviceTexts.statusActive : serviceTexts.statusHidden}
                  </p>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 text-sm text-[color:var(--foreground-soft)]">
                    ...
                    <span className="rounded-full border border-[color:var(--border-soft)] bg-[color:var(--surface-strong)] px-3 py-1.5">
                      {serviceTexts.price}: {service.price == null ? serviceTexts.pricePlaceholder : `${service.price} ₽`}
                    </span>
                  </div>
                </div>
              </div>

              <form action={updateServiceAction} className="mt-6 grid gap-4 xl:grid-cols-[1.3fr_180px_180px_180px_180px]">
                ...
                <button type="submit" className="rounded-full border border-[color:var(--border)] px-5 py-3 text-sm font-medium transition hover:border-[color:var(--accent)] hover:text-[color:var(--accent)]">
                  {dict.common.save}
                </button>
              </form>
            ...
            <article key={master.id} className="overflow-hidden rounded-[2rem] border border-[color:var(--border)] bg-white shadow-[var(--shadow-md)]">
              ...
              <div className="p-6">
                ...
                <form action={updateMasterAction} className="mt-6 space-y-6">
                  ...
                  <div className="rounded-[1.6rem] border border-[color:var(--border-soft)] bg-[color:var(--surface-muted)] p-5">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                      <div>
                        <p className="text-sm uppercase tracking-[0.2em] text-[color:var(--muted)]">
                          {locale === "ru" ? "Расписание" : "Schedule"}
                        </p>
                        <h4 className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-[color:var(--foreground)]" style={{ fontFamily: "var(--font-heading)" }}>
                          {locale === "ru" ? "Рабочие часы мастера" : "Staff working hours"}
                        </h4>
                      </div>
                    ...
                    <div className="mt-5 grid gap-3">
                      {master.weeklySchedule.map((day) => (
                        <div key={day.dayOfWeek} className="grid gap-3 rounded-[1.3rem] border border-[color:var(--border-soft)] bg-white px-4 py-4 xl:grid-cols-[1.1fr_180px_180px_180px] xl:items-center">
                          <div>
                            <p className="text-sm font-medium text-[color:var(--foreground)]">{weekdayLabels[day.dayOfWeek]}</p>
                          </div>
                          ...
                          >
                            <option value="true">{locale === "ru" ? "Рабочий день" : "Working day"}</option>
                            <option value="false">{locale === "ru" ? "Выходной" : "Day off"}</option>
                          </select>
